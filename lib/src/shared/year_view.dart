...
class YearView extends StatelessWidget {
  ...
  YearView({
    ...
    assert(() {
      return (displayedYearRange.end.year - displayedYearRange.start.year) ==
          (yearsInOnePage - 1);
    }(), "the display year range must always be $yearsInOnePage years.");
  ...
  Widget build(BuildContext context) {
    ...
    while (i < numberOfYears) {
      ...
      if (isDisabled) {
        yearWidget = ExcludeSemantics(
          child: yearWidget,
        );
      } else {
        final date = DateTime(yearsName[i]);
        yearWidget = InkResponse(
      ...
      yearWidgetsList.add(yearWidget);
      i++;
    }
